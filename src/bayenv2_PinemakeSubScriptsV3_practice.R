...
for (j in 380){  
 ...
 write(	c("#!/bin/bash", "#PBS -r n", "#PBS -m a", "#PBS -M klott@zoology.ubc.ca", "#PBS -l walltime=00:20:00", "#PBS -l procs=1", "#PBS -l mem=1gb", "" ,"cd $PBS_O_WORKDIR", paste("R --vanilla < bayenv2_PinecallSNPsV3.R --args", startSNP, endSNP, j, sep=" ")), ncolumns=1, append=FALSE, file=SubmitScriptName)
